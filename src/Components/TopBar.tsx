...
const notifs = [
  { id: 1, text: 'تم إضافة 5 محاضرات جديدة في مادة برمجة 1',   time: '10 دقائق', icon: '📄', color: '#3B82F6', read: false },
  { id: 2, text: 'امتحان منتصف الترم القادم بعد 3 أيام',          time: '1 ساعة',   icon: '📋', color: '#F59E0B', read: false },
  { id: 3, text: 'تم رفع حل واجب المصفوفات',                     time: '3 ساعات',  icon: '📝', color: '#10B981', read: true  },
  { id: 4, text: 'مادة جديدة متاحة: Advanced Algorithms',         time: 'أمس',      icon: '📚', color: '#8B5CF6', read: true  },
]
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {breadcrumbs.map((crumb, i) => (
            <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <button onClick={crumb.onClick} style={{
                background: 'none', border: 'none', cursor: crumb.onClick ? 'pointer' : 'default', padding: 0,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ position: 'relative' }}>
            <span style={{ position: 'absolute', right: 11, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', fontSize: 14 }}>🔍</span>
            <input
              value={search} onChange={e => setSearch(e.target.value)}
              placeholder="ابحث عن مادة، ملف، محاضرة..."
              style={{
                background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-medium)',
                borderRadius: 11, padding: '7px 36px 7px 60px',
                color: 'var(--text-primary)', fontSize: 13, width: 270, outline: 'none', direction: 'rtl'
              }}
            />
            <span style={{
              position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)',
              background: 'rgba(255,255,255,0.08)', color: 'var(--text-muted)',
              fontSize: 10, borderRadius: 6, padding: '2px 5px', fontFamily: 'monospace'
            }}>Ctrl K</span>
          </div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            {showNotifs && (
              <div style={{
                ...
                width: 320, background: '#131C2E',
                border: '1px solid var(--border-medium)', borderRadius: 16, overflow: 'hidden',
                ...
                <div style={{ padding: '13px 16px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>الإشعارات {unread > 0 && <span style={{ fontSize: 11, background: '#3B82F6', color: '#fff', borderRadius: 8, padding: '1px 6px', marginRight: 4 }}>{unread}</span>}</span>
                  <button onClick={() => setNotifList(n => n.map(x => ({ ...x, read: true })))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--accent-blue)', fontSize: 12 }}>تحديد الكل مقروء</button>
                </div>
                {notifList.map(n => (
                  <div key={n.id}
                    ...
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: n.color+'20', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15 }}>{n.icon}</div>
                    <div style={{ flex: 1 }}>
                    ...
                    {!n.read && <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#3B82F6', flexShrink: 0, marginTop: 5 }} />}
                  </div>
                ))}
                <div style={{ padding: '10px 16px', textAlign: 'center' }}>
                  <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--accent-blue)', fontSize: 12 }}>عرض كل الإشعارات ←</button>
                </div>
              </div>
            )}
          </div>
      ...
      {title && (
        <div style={{ marginTop: 10 }}>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: 'var(--text-primary)' }}>{title}</h1>
